...
export const StatsCommand = effectCmd({
  command: "stats",
  describe: t("stats.show_token_usage_and_cost_statistics"),
  builder: (yargs) =>
    yargs
      .option("days", {
        describe: t("stats.show_stats_for_the_last_n_days_default_all_time"),
        type: "number",
      })
      .option("tools", {
        describe: t("stats.number_of_tools_to_show_default_all"),
        type: "number",
      })
      .option("models", {
        describe: t("stats.show_model_statistics_default_hidden_pass_a_number"),
      })
      .option("project", {
        describe: t("stats.filter_by_project_default_all_projects_empty_strin"),
        type: "string",
      }),
...
const aggregateSessionStats = Effect.fn("Cli.stats.aggregate")(function* (
  ...
  if (filteredSessions.length > 1000) {
    console.log(t("stats.large_dataset_detected_sessions_this_may_take_a_wh", { filteredSessions_length: filteredSessions.length }))
  }
...
export function displayStats(stats: SessionStats, toolLimit?: number, modelLimit?: number) {
  ...
  console.log("├────────────────────────────────────────────────────────┤")
  console.log(renderRow(t("stats.sessions"), stats.totalSessions.toLocaleString()))
  console.log(renderRow(t("stats.messages"), stats.totalMessages.toLocaleString()))
  console.log(renderRow(t("stats.days"), stats.days.toString()))
  console.log("└────────────────────────────────────────────────────────┘")
  console.log()
  ...
  console.log(renderRow(t("stats.total_cost"), `$${cost.toFixed(2)}`))
  console.log(renderRow(t("stats.avg_costday"), `$${costPerDay.toFixed(2)}`))
  console.log(renderRow(t("stats.avg_tokenssession"), formatNumber(Math.round(tokensPerSession))))
  const medianTokensPerSession = isNaN(stats.medianTokensPerSession) ? 0 : stats.medianTokensPerSession
  console.log(renderRow(t("stats.median_tokenssession"), formatNumber(Math.round(medianTokensPerSession))))
  console.log(renderRow(t("stats.input"), formatNumber(stats.totalTokens.input)))
  console.log(renderRow(t("stats.output"), formatNumber(stats.totalTokens.output)))
  console.log(renderRow(t("stats.cache_read"), formatNumber(stats.totalTokens.cache.read)))
  console.log(renderRow(t("stats.cache_write"), formatNumber(stats.totalTokens.cache.write)))
  console.log("└────────────────────────────────────────────────────────┘")
  console.log()
  ...
  if (modelLimit !== undefined && Object.keys(stats.modelUsage).length > 0) {
    ...
    for (const [model, usage] of modelsToDisplay) {
      ...
      console.log(renderRow(t("stats.messages_1"), usage.messages.toLocaleString()))
      console.log(renderRow(t("stats.input_tokens"), formatNumber(usage.tokens.input)))
      console.log(renderRow(t("stats.output_tokens"), formatNumber(usage.tokens.output)))
      console.log(renderRow(t("stats.cache_read_1"), formatNumber(usage.tokens.cache.read)))
      console.log(renderRow(t("stats.cache_write_1"), formatNumber(usage.tokens.cache.write)))
      console.log(renderRow(t("stats.cost"), `$${usage.cost.toFixed(4)}`))
      console.log("├────────────────────────────────────────────────────────┤")
    }
